...
document.getElementById("signupBtn").onclick = async () => {
  try {
    authMsg.textContent = "";
    const name = nameInput.value.trim() || "Khambir User";
...
function renderChats(chats) {
  ...
  chats
    .filter((c) => (c.name || "Private Chat").toLowerCase().includes(filter))
    .sort((a, b) => (b.updatedAt?.seconds || 0) - (a.updatedAt?.seconds || 0))
    .forEach((chat) => {
      const div = document.createElement("div");
      div.className = "chat-item" + (activeChat?.id === chat.id ? " active" : "");
      div.innerHTML =
        '<div class="avatar">' + (chat.name || "C")[0].toUpperCase() + '</div>' +
        '<div class="chat-meta"><h3>' + escapeHtml(chat.name || "Private Chat") + '</h3>' +
        '<p>' + escapeHtml(chat.lastMessage || "No messages yet") + '</p></div>';
...
async function createPrivateChat() {
  ...
  if (usersSnap.empty) {
    alert("No user found. Ask them to sign up first.");
    return;
  }
  ...
  await addDoc(collection(db, "chats"), {
    ...
    memberEmails: [currentUser.email, other.email],
    createdAt: serverTimestamp(),
...
async function createGroupChat() {
  ...
  const emails = prompt("Member emails separated by comma. They must sign up first:");
  if (!emails) return;
...
function openChat(chat) {
  ...
  chatSubtitle.textContent = chat.type === "group" ? "Group chat" : "Private chat";
...
function listenToMessages(chatId) {
  ...
  unsubscribeMessages = onSnapshot(q, (snap) => {
    ...
    snap.forEach((d) => {
      ...
      div.innerHTML =
        (!mine ? '<div class="sender">' + escapeHtml(m.senderName || "User") + '</div>' : "") +
        escapeHtml(m.text || "") +
